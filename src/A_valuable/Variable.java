...
public class Variable {
	...
	public static void main(String[] args) {
		...
		_byte = 127; //타입에 맞는 값을 저장해야 한다. //byte
		_short = 30000;	                   		 //short	
		_int = 20;								 //int
		_long = 900L; //long 접미사 : L			 //long
		_float = 3.14f; //float 접미사 : f			 //float	
		_double = 3.14; //double 접미사 : d(생략가능) //double
		_char = '가';							 //char
		...
		System.out.println(_byte); //128 -> -128    129 -> -127 (오버플로우)
		...
		//표현범위가 작은 타입에서 큰 타입으로 형변환 할 경우 데이터 손실될 염려가 없다.
		//그러므로 형변환을 생략할 수 있다.
		_int = _byte;
		_long = _int;
		_double = _int;
...
//		System.out.println("입력받은 숫자 : "+ input2);
		...
		System.out.println("문자열과 숫자를 더해주세요");
		String input1 = s.nextLine();
		int input2 = Integer.parseInt(s.nextLine());
